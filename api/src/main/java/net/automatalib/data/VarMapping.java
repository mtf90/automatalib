...
public class VarMapping<K extends SymbolicDataValue<?>, V extends SymbolicDataValue<?>> extends Mapping<K, V> {
    ...
    public VarMapping(K k1, V v1, K k2, V v2) {
        put(k1, v1);
        put(k2, v2);
    }

    public VarMapping(K k1, V v1, K k2, V v2, K k3, V v3) {
        put(k1, v1);
        put(k2, v2);
        put(k3, v3);
    }
    ...
    public static class GeneratorMapping extends Mapping<DataType<?>, FreshValueGenerator<?>> {

        @SuppressWarnings("unchecked")
        public <T> FreshValueGenerator<T> get(DataType<T> key) {
            return (FreshValueGenerator<T>) super.get(key);
        }
    }
